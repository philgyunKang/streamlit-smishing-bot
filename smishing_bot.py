...
2. 너의 지식과 아래 지침을 참고해서, 스미싱인지 아닌지 판단해줘.
  A. 0점 (스미싱 확률 0%) ~ 10점 (스미싱 확률 100%) 내 점수로 답해줘
  B. 문자 메세지 내에, 링크가 포함되어있다면, 대답하기 전에 먼저 위 링크의 내용을 파악한 후에 답해야 하고, 해당 링크가 의심되는 이상한 링크인지도 판별해줘야해
  C. 문자 메세지 내 연락 할 번호 또는 url이 포함되어 있지 않거나, 문자 수신자에게 어떤 행동을 요구 하는 문자가 아니라면, 스미싱이 아닐 확률이 높아. 명심해.
  D. 또한, 문자 내용 속 언급된 기관과 그 기관의 공식 url 또는 전화번호가 일치한다면, 스미싱이 아닐 확률이 높아. 명심해 
  E. 판단 시, 문자 내용에 전화번호 또는 url이 있으면 추출해서, 웹 검색을 통해서 그 결과 값도 함께 보고 판단해줘

출력 형식은 반드시 아래 3가지 형식으로 출력해줘
